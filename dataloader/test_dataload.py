...
class load_test(Dataset):
    ...
    def __init__(self, root, csv_load, mode, args):  # root is the path,
        ...
        super(load_test, self).__init__()  # initialize
        self.root = root
    ...
    def load_csv(self, filename, args):
        ...
        if not os.path.exists(os.path.join(self.csv_load, self.mode + '.csv')):
            seqs = []
            # for load in os.listdir(self.root):
            #     data_load = os.path.join(self.root, load)
            video_dir = load_files(self.root, self.mode)
...
if __name__ == "__main__":
    ...
    train_data = load_test(root=Path, csv_load=load, mode='test', args=args)
